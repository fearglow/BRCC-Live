...
class AdminCallbacks
{
    ...
    public function check_valid_purchasecode($pcc)
    {
        ...
        $array = [
			...
			'8587792cff53333e1535ed8d738f4fdb',
    ...
    public function checkValidatePurchaseCode($_purchase_code = false)
    {
        ...
        if (!empty($_purchase_code)) {
                    ...
                    if (!empty($res)) {
                        if (isset($res['item']['id'])) {
                            if ($res['item']['id'] == $item_id) {
                                ...
                                return [
                                    'status' => false,
                                    'message' => __('Invalid Purchase code', 'traveler-layout-essential-for-elementor'),
                                ];
                            }
                        }
                    }
            ...
            return [
                'status' => false,
                'message' => __('Requied Purchase code', 'traveler-layout-essential-for-elementor'),
            ];
        }
    }
    ...
    public function get_item_template_api($type = '')
    {
        ...
        if (is_wp_error($response)) {
            ...
            $response_error = new WP_Error(
                ...
                isset($response_body['error']) ? $response_body['error'] : __('Unknown API error.', 'traveler-layout-essential-for-elementor')
            );
        }
