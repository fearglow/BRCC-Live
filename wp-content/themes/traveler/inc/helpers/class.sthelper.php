    ...
    function st_get_avatar_in_list_service($post_id, $size = 35, $return_link = false) {
        ...
        if (st()->get_option('avatar_in_list_service', 'off') == 'on') {
            ...
            if ($return_link) {
                $user_link = get_permalink(st()->get_option('user_info_page'));
                $user_link = add_query_arg('user', $post_author_id, $user_link);
                return '<a href="' . $user_link . '" target="_blank" class="author-link">' . st_get_profile_avatar($post_author_id, $size) . '</a>';
            }
            return '<a href="' . $author_link . '" target="_blank" title="' . get_the_author() . '" class="author-link">' . st_get_profile_avatar($post_author_id, $size) . '</a>';
        ...
        if (!empty($data['guest_name'])) {
            ...
            <div class="col-md-12">
                <div class="item_booking_detail">
                    <strong><?php esc_html_e('Guest Name and Weight:', 'traveler') ?></strong>
                    <?php
                    $guest_weights = isset($data['guest_title']) ? $data['guest_title'] : [];
                    $weight_labels = [
                        '225' => 'Up to 225 lbs',
                        '300' => 'Up to 300 lbs'
                    ];
                    ...
                    foreach ($data['guest_name'] as $k => $name) {
                        $weight = isset($guest_weights[$k]) ? $weight_labels[$guest_weights[$k]] . ' - ' : '';
                        $str = $weight . $name;
                        $html[] = $str;
                    }
                    echo implode(', ', $html);
...
if (!function_exists('st_admin_print_order_item_guest_name')) {
    function st_admin_print_order_item_guest_name($data) {
        if (!empty($data['guest_name'])) {
            ?>
            <div class="form-row">
                <label class="form-label" for=""><?php esc_html_e('Guest Name and Weight', 'traveler') ?></label>
                <div class="controls">
                    <?php
                    $guest_weights = isset($data['guest_title']) ? $data['guest_title'] : [];
                    $weight_labels = [
